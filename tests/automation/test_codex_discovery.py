...
def test_discover_ready_candidate():
    adapter = FakeProjectAdapter(
        project_number=1,
        statuses=["BACKLOG", "READY", "BUILDING", "TESTING", "REVIEWING", "FIXING", "HUMAN_REVIEW"],
        items=[{"project_item_id": 11, "issue_number": 5, "status": "READY", "assignee": "me", "repo": "org/repo", "title": "T", "body": "x"}],
    )
...
def test_non_ready_excluded():
    adapter = FakeProjectAdapter(
        ...
        items=[{"project_item_id": 12, "issue_number": 6, "status": "BACKLOG", "assignee": "me"}],
    )
    candidates = discover_ready_candidates(adapter, 1, "me")
    assert candidates == []
...
def test_zero_ready_returns_empty():
    adapter = FakeProjectAdapter(project_number=1, statuses=["BACKLOG", "READY"], items=[])
    candidates = discover_ready_candidates(adapter, 1, "me")
    assert candidates == []
...
def test_multiple_candidates_sorted():
    adapter = FakeProjectAdapter(
        ...
        statuses=["BACKLOG", "READY"],
        items=[
            {"project_item_id": 21, "issue_number": 20, "status": "READY", "assignee": "me"},
            {"project_item_id": 22, "issue_number": 2, "status": "READY", "assignee": "me"},
        ],
    )
    candidates = discover_ready_candidates(adapter, 1, "me")
...
def test_malformed_item_raises():
    adapter = FakeProjectAdapter(project_number=1, statuses=["READY"], items=[{"bad": "item"}])
    try:
...
def test_claim_and_hand_off_to_controller():
    adapter = FakeProjectAdapter(
        ...
        items=[{"project_item_id": 31, "issue_number": 7, "status": "READY", "assignee": "me", "repo": "org/repo"}],
    )
    ...
    result = process_once(1, adapter, orchestrator, os.environ["CODEX_WORKER_ID"], "run-1", task_store=None, execute=True, repo="org/repo")
...
def test_claim_conflict_prevents_execution():
    adapter = FakeProjectAdapter(
        project_number=1,
        statuses=["READY", "BUILDING"],
        items=[{"project_item_id": 32, "issue_number": 8, "status": "READY", "assignee": "me", "repo": "org/repo"}],
    )
    orchestrator = Orchestrator(
        builder=make_handler(StageOutcome.SUCCESS),
        tester=make_handler(StageOutcome.SUCCESS),
    ...
    os.environ["CODEX_WORKER_ID"] = "me"
    result = process_once(1, adapter, orchestrator, os.environ["CODEX_WORKER_ID"], "run-2", task_store=store, execute=True, repo="org/repo")
...
def test_discovery_does_not_mutate_task_state_directly():
    adapter = FakeProjectAdapter(
        project_number=1,
        statuses=["READY", "BUILDING"],
        items=[{"project_item_id": 33, "issue_number": 9, "status": "READY", "assignee": "me", "repo": "org/repo"}],
    )
    store = InMemoryTaskStore()
    # ensure discovery path uses store but does not mutate state before claim
    _ = store.create_if_missing("org/repo#9")
    os.environ["CODEX_WORKER_ID"] = "me"
    process_once(1, adapter, None, os.environ["CODEX_WORKER_ID"], "run-3", task_store=store, execute=False, repo="org/repo")
